...
class UserSecurityRequest(MedplumFHIRBase):
    """User security request for the 'forgot password' flow, email verification, etc."""

    resource_type: Literal["UserSecurityRequest"] = Field(
        default="UserSecurityRequest",
        alias="resourceType"
    )
    ...
    type: Optional[Literal['invite', 'verify-email', 'reset']] = Field(default=None, description="The type of user security request.")
    user: Reference = Field(default=..., description="The user performing the security request.")
    secret: str = Field(default=..., description="Secret string used to verify the identity of the user.")
    used: Optional[bool] = Field(default=None, description="Whether this request has been used, and is therefore no longer valid.")
    redirect_uri: Optional[str] = Field(default=None, alias="redirectUri", description="Redirect URI used when redirecting a client back to the client application.")
...
if not TYPE_CHECKING:
    ...
    register_model("UserSecurityRequest", UserSecurityRequest)
