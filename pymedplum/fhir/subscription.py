...
class Subscription(MedplumFHIRBase):
    ...
    status: Literal['requested', 'active', 'error', 'off'] = Field(default=..., description="The status of the subscription, which marks the server state for managing the subscription.")
    ...
    end: Optional[str] = Field(default=None, description="The time for the server to turn the subscription off.")
...
class SubscriptionChannel(MedplumFHIRBase):
    ...
    type: Literal['rest-hook', 'websocket', 'email', 'sms', 'message'] = Field(default=..., description="The type of channel to send notifications on.")
    endpoint: Optional[str] = Field(default=None, description="The url that describes the actual end-point to send messages to.")
    payload: Optional[str] = Field(default=None, description="The mime type to send the payload in - either application/fhir+xml, or application/fhir+json. If the payload is not present, then there is no payload in the notification, just a notification. The mime type &quot;text/plain&quot; may also be used for Email and SMS subscriptions.")
