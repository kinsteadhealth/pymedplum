...
class Linkage(MedplumFHIRBase):
    ...
    resource_type: Literal["Linkage"] = Field(
        default="Linkage",
        alias="resourceType"
    )
    ...
    active: Optional[bool] = Field(default=None, description="Indicates whether the asserted set of linkages are considered to be &quot;in effect&quot;.")
    author: Optional[Reference] = Field(default=None, description="Identifies the user or organization responsible for asserting the linkages as well as the user or organization who establishes the context in which the nature of each linkage is evaluated.")
    item: list[LinkageItem] = Field(default=..., description="Identifies which record considered as the reference to the same real-world occurrence as well as how the items should be evaluated within the collection of linked items.")


class LinkageItem(MedplumFHIRBase):
    ...
    type: Literal['source', 'alternate', 'historical'] = Field(default=..., description="Distinguishes which item is &quot;source of truth&quot; (if any) and which items are no longer considered to be current representations.")
    resource: Reference = Field(default=..., description="The resource instance being linked as part of the group.")
