...
if TYPE_CHECKING:
    ...
    from pymedplum.fhir.parameters import Parameters


class AsyncJob(MedplumFHIRBase):
    """Contains details of long running asynchronous/background jobs."""

    resource_type: Literal["AsyncJob"] = Field(default="AsyncJob", alias="resourceType")
    ...
    output: Parameters | None = Field(
        default=None, description="Outputs resulting from the async job."
    )
    type: Literal["data-migration"] | None = Field(
        default=None, description="The type of the AsyncJob."
    )
    data_version: int | float | None = Field(
        default=None,
        alias="dataVersion",
        description="The data version of the migration this job represents.",
    )
    min_server_version: str | None = Field(
        default=None,
        alias="minServerVersion",
