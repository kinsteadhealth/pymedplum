...
class AsyncJob(MedplumFHIRBase):
    """Contains details of long running asynchronous/background jobs."""

    resource_type: Literal["AsyncJob"] = Field(
        default="AsyncJob",
        alias="resourceType"
    )
    ...
    request: str = Field(default=..., description="The full URL of the original kick-off request. In the case of a POST request, this URL will not include the request parameters.")
    output: Optional[Parameters] = Field(default=None, description="Outputs resulting from the async job.")
    type: Optional[Literal['data-migration']] = Field(default=None, description="The type of the AsyncJob.")
    data_version: Optional[Union[int, float]] = Field(default=None, alias="dataVersion", description="The data version of the migration this job represents.")
    min_server_version: Optional[str] = Field(default=None, alias="minServerVersion", description="The minimum Medplum server version required to run this job.")
...
if not TYPE_CHECKING:
    ...
    register_model("AsyncJob", AsyncJob)
