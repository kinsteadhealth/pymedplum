...
class BulkDataExport(MedplumFHIRBase):
    ...
    status: Literal['accepted', 'active', 'completed', 'error', 'cancelled'] = Field(default=..., description="The status of the request.")
    request_time: str = Field(default=..., alias="requestTime", description="Indicates the server's time when the query is requested.")
    transaction_time: Optional[str] = Field(default=None, alias="transactionTime", description="Indicates the server's time when the query is run. The response SHOULD NOT include any resources modified after this instant, and SHALL include any matching resources modified up to and including this instant.")
    request: str = Field(default=..., description="The full URL of the original Bulk Data kick-off request. In the case of a POST request, this URL will not include the request parameters.")
    requires_access_token: Optional[bool] = Field(default=None, alias="requiresAccessToken", description="Indicates whether downloading the generated files requires the same authorization mechanism as the $export operation itself.")
    output: Optional[List[BulkDataExportOutput]] = Field(default=None, description="An array of file items with one entry for each generated file. If no resources are returned from the kick-off request, the server SHOULD return an empty array.")
    deleted: Optional[List[BulkDataExportDeleted]] = Field(default=None, description="An array of deleted file items following the same structure as the output array.")
    error: Optional[List[BulkDataExportError]] = Field(default=None, description="Array of message file items following the same structure as the output array.")
...
class BulkDataExportDeleted(MedplumFHIRBase):
    ...
    type: ResourceType = Field(default=..., description="The FHIR resource type that is contained in the file.")
    url: str = Field(default=..., description="The absolute path to the file. The format of the file SHOULD reflect that requested in the _outputFormat parameter of the initial kick-off request.")
...
if not TYPE_CHECKING:
    ...
    register_model("BulkDataExportError", BulkDataExportError)
    register_model("BulkDataExportOutput", BulkDataExportOutput)
