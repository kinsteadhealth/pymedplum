...
class DomainConfiguration(MedplumFHIRBase):
    """Domain specific configuration for the Medplum application."""

    resource_type: Literal["DomainConfiguration"] = Field(
        default="DomainConfiguration",
        alias="resourceType"
    )
    ...
    domain: str = Field(default=..., description="Globally unique domain name for this configuration.")
    identity_provider: Optional[IdentityProvider] = Field(default=None, alias="identityProvider", description="Optional external Identity Provider (IdP) for the domain name.")
...
if not TYPE_CHECKING:
    ...
    register_model("DomainConfiguration", DomainConfiguration)
