...
class BiologicallyDerivedProduct(MedplumFHIRBase):
    ...
    product_code: Optional[CodeableConcept] = Field(
        default=None,
        alias="productCode",
        description="A code that identifies the kind of this biologically derived product (SNOMED Ctcode).",
    )
    status: Optional[Literal["available", "unavailable"]] = Field(
...
class BiologicallyDerivedProductStorage(MedplumFHIRBase):
    ...
    description: Optional[str] = Field(
        default=None, description="Description of storage."
    )
    temperature: Optional[Union[int, float]] = Field(
        default=None, description="Storage temperature."
    )
    scale: Optional[Literal["farenheit", "celsius", "kelvin"]] = Field(
